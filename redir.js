(function() {
    ...
    // Дефолтный сервер - my.bylampa.online
    var DEFAULT_SERVER = 'my.bylampa.online';
    ...
    // ============ ДОБАВЛЯЕМ ИНИЦИАЛИЗАЦИЮ ДЕФОЛТНОГО СЕРВЕРА ============
    // Проверяем, есть ли сохраненный сервер, если нет - устанавливаем дефолтный
    ...
    function startMe() {
        ...
        // ============ ПУНКТ 3: ИСТОРИЯ СЕРВЕРОВ ============
        // Безопасно добавляем историю без изменения логики
        
        // Функция для сохранения сервера в историю
        function addToHistory(server) {
            ...
            // Если сервер уже есть в истории, удаляем его из старой позиции
        ...
        // Добавляем текущий сервер в историю при старте
        ...
        // ============ ПУНКТ 4: БЫСТРЫЙ ПЕРЕКЛЮЧАТЕЛЬ ============
        // Добавляем кнопку быстрого переключения между серверами из истории
        ...
        function addQuickSwitcher() {
            ...
            // Если в истории меньше 2 серверов - не показываем кнопку
            ...
            // Вставляем после кнопки редиректа
            ...
            $('#SERVER_SWITCHER').on('hover:enter hover:click hover:touch', function() {
                ...
                if (history.length < 2) {
                    Lampa.Noty.show('Нужно минимум 2 сервера в истории');
                    return;
                }
                ...
                // Если текущий сервер не найден в истории, берем первый
                if (currentIndex === -1) {
                    currentIndex = 0;
                }
                
                // Вычисляем следующий сервер (циклически)
                ...
                // Перезапускаем плагин для обновления кнопки редиректа
                $('#REDIRECT').remove();
                startMe();
            });
            
            // Добавляем обработчик для долгого нажатия - показать список всех серверов
            var pressTimer;
            $('#SERVER_SWITCHER').on('hover:enter', function() {
                pressTimer = setTimeout(function() {
                    var history = JSON.parse(Lampa.Storage.get('server_history') || '[]');
                    if (history.length > 0) {
                        // Показываем список через Noty (простое уведомление)
                        var msg = 'Серверы в истории:\n' + history.join('\n');
        ...
        // Вызываем добавление переключателя
        addQuickSwitcher();
        // ===================================================
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: {
                ...
                type: 'input', 
                values: '',
                placeholder: 'Например: ' + DEFAULT_SERVER, // Показываем пример с дефолтным
                default: DEFAULT_SERVER // Устанавливаем дефолт
            },
            field: {
                name: 'Адрес сервера',
            ...
            onChange: function (value) {
                // Сохраняем в историю при изменении
                if (value && value.trim() !== '') {
                    addToHistory(value);
                }
