(function() {
    ...
    // ВАЖНО: Всегда используем HTTP, даже если Lampa загружена по HTTPS
    ...
    // Инициализация дефолтного сервера (без протокола, только домен)
    ...
    function startMe() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: {
                name: 'Адрес сервера',
                description: 'Вводите ТОЛЬКО домен (без http://) - протокол HTTP будет добавлен автоматически'
            },
            onChange: function (value) {
                if (value && value.trim() !== '') {
                    // Убираем возможные протоколы из введенного значения
