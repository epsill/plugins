(function() {
    ...
    function switchServer() {
        ...
        if (!Array.isArray(history) || history.length < 2) {
            Lampa.Noty.show('Добавьте второй сервер в настройках');
            return;
        }
        ...
        Lampa.Noty.show('✓ Сервер: ' + nextServer, {timeout: 2000});
    }

    // ============ ПРОСТАЯ ОЧИСТКА: ДОЛГОЕ НАЖАТИЕ ============
    function clearAllHistory() {
        saveHistory([]);
        Lampa.Storage.set('location_server', ''); // Сбрасываем текущий сервер
        $('#REDIRECT').remove(); // Убираем кнопку редиректа
        $('#SERVER_SWITCHER').attr('title', 'История очищена');
        Lampa.Noty.show('🧹 Вся история серверов очищена!', {timeout: 3000});
    }
    // ======================================================
    ...
    function startMe() {
        try {
            if (!window.redirect_initialized) {
                ...
                // Кнопка переключения
                var switcherBUTT = '<div id="SERVER_SWITCHER" class="head__action selector server-switcher" title="Клик - след. сервер, ДОЛГОЕ НАЖАТИЕ - очистить всё">' + 
                ...
                // ОБРАБОТЧИК: Короткое нажатие - переключение
                $('#SERVER_SWITCHER').on('hover:enter hover:click hover:touch', function(e) {
                    // Защита от ложного срабатывания при долгом нажатии
                    if (e && e.type === 'click' && e.originalEvent && e.originalEvent.isLongPress) {
                        return;
                    }
                    e.preventDefault();
                    e.stopPropagation();
                ...
                // ОБРАБОТЧИК: Долгое нажатие - ОЧИСТКА ВСЕГО
                var pressTimer;
            ...
            if (currentServer && typeof currentServer === 'string' && currentServer.trim() !== '') {
                addToHistory(currentServer);
                $('#SERVER_SWITCHER').attr('title', 'Текущий сервер: ' + currentServer + ' | Долгое нажатие = очистка');
            }
            
            // Добавляем настройки (один раз)
                ...
                Lampa.SettingsApi.addParam({
                    ...
                    onChange: function(value) {
                        ...
                        if (value == '') {
                            ...
                            $('#SERVER_SWITCHER').attr('title', 'Текущий сервер: ' + value + ' | Долгое нажатие = очистка');
                        }
                    }
                });
