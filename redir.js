(function() {
    ...
    function removeLastFromHistory() {
        var history = getHistory();
        if (history.length > 0) {
            var removed = history.shift(); // Удаляем первый (самый старый)
            saveHistory(history);
    ...
    function removeCurrentFromHistory() {
        ...
        if (index !== -1) {
            ...
            // Если после удаления история не пуста - переключаем на первый
            if (history.length > 0) {
                Lampa.Storage.set('location_server', history[0]);
                $('#SERVER_SWITCHER').attr('title', 'Текущий сервер: ' + history[0]);
            }
        }
    }
    ...
    function switchServer() {
        ...
        if (!Array.isArray(history) || history.length < 2) {
            history = [current];
            if (current !== DEFAULT_SERVER) history.push(DEFAULT_SERVER);
            else history.push('bylampa.online');
            saveHistory(history);
        }
    ...
    function startMe() {
        if (!window.redirect_initialized) {
            ...
            $('#REDIRECT').on('hover:enter hover:click hover:touch', function(e) {
                ...
                if (server) window.location.href = server_protocol + server;
            });
        ...
        if (!Lampa.Storage.get('location_server')) {
            setTimeout(function(){ $('#REDIRECT').remove(); }, 10);
        }
        ...
        if (!window.settings_added) {
            ...
            Lampa.SettingsApi.addParam({
                ...
                onChange: function(value) {
                    if (value && value.trim() !== '') addToHistory(value);
                    if (value == '') $('#REDIRECT').remove();
                    else $('#SERVER_SWITCHER').attr('title', 'Текущий сервер: ' + value);
                }
            });
            ...
            // ============ НОВЫЙ РАЗДЕЛ: УПРАВЛЕНИЕ ИСТОРИЕЙ ============
            Lampa.SettingsApi.addParam({
                component: 'location_redirect',
                param: {
                    name: 'history_header',
                    type: 'header',
                    default: 'Управление историей серверов'
                },
                field: {
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    ...
                    description: 'Удаляет первый сервер из истории'
                },
                onChange: function() {
