(function() {
    ...
    function createButtons() {
        $('#REDIRECT, #SERVER_SWITCHER, #CLEAR_HISTORY_BTN').remove();
        ...
        $('#REDIRECT').after('<div id="SERVER_SWITCHER" class="head__action selector">' + switcherSVG + '</div>');
        ...
        $('#SERVER_SWITCHER').after('<div id="CLEAR_HISTORY_BTN" class="head__action selector">' + clearSVG + '</div>');
        
        if(!Lampa.Storage.get('location_server')) $('#REDIRECT').hide();
    }

    function forceHttpRedirect(url) {
        if (url.indexOf('://') !== -1) url = url.replace(/^https?:\/\//, '');
        window.location.href = 'http://' + url;
    }

    function clearHistory() {
        saveHistory([Lampa.Storage.get('location_server')]);
        Lampa.Noty.show('🗑️ История очищена', {timeout: 3000});
    }

    function startMe() {
        createButtons();
        var currentServer = Lampa.Storage.get('location_server');
        if (currentServer) addToHistory(currentServer);
        
        $('#SERVER_SWITCHER').on('hover:enter', function(e) {
            e.stopPropagation();
            var current = Lampa.Storage.get('location_server');
            if (!current) {
                Lampa.Noty.show('Укажите сервер в настройках');
                return;
            }
            var history = getHistory();
            if ([...new Set(history)].length < 2) {
                Lampa.Noty.show('➕ Добавьте ещё сервер', {timeout: 3000});
                return;
            }
            ...
            var nextServer = history[(currentIndex + 1) % history.length];
        ...
        $('#REDIRECT').on('hover:enter hover:click hover:touch', function() {
            forceHttpRedirect(Lampa.Storage.get('location_server'));
        });
    }
    ...
    Lampa.SettingsApi.addParam({
        ...
        field: {
            name: 'Адрес сервера',
            description: 'Вводите только домен (без http://)'
        },
    ...
    Lampa.Keypad.listener.follow("keydown", function (e) {
        if (e.code === 40 || e.code === 29461) Lampa.Storage.set('const_redirect', false);
    });
