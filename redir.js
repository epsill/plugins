(function() {
    ...
    // Просто строка, без точек в названии переменной
    var DEFAULT_SERVER_TEXT = 'my.bylampa.online';
    ...
    // Инициализация дефолтного сервера - БЕЗ ТОЧЕК В НАЗВАНИИ ПЕРЕМЕННОЙ
    if (!Lampa.Storage.get('location_server')) {
        Lampa.Storage.set('location_server', 'my.bylampa.online');
    }
    ...
    function removeLastFromHistory() {
        var history = getHistory();
        if (history.length > 0) {
            var removed = history.pop(); // Удаляем последний (самый старый)
            saveHistory(history);
    ...
    function switchServer() {
        ...
        if (!Array.isArray(history) || history.length < 2) {
            history = [current];
            if (current !== 'my.bylampa.online') {
                history.push('my.bylampa.online');
            } else {
            ...
            if (!window.settings_added) {
                ...
                Lampa.SettingsApi.addParam({
                    ...
                    param: {
                        ...
                        type: 'input',
                        placeholder: 'Например: my.bylampa.online',
